...
router.post('/item', async (req, res) => {
    ...
    try{
      newItem = await new Warehouse().addItem(new Item(req.body.description, req.body.price, req.body.supplierId, req.body.SKUId));
      if (newItem === 422) {
        return res.status(422).end();
      }
  ...
  router.put('/item/:id', async (req, res) => {
    ...
    try{
      updatedItem = await new Warehouse().editItem(req.params.id, req.body.newDescription, req.body.newPrice);
      if (updatedItem === 404) {
          return res.status(404).end();
      }
